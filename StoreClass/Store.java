...
public class Store
{
	...
	{
        int hoursOpen = storeClose - storeOpen;
        return hoursOpen;
	}
	...
	public boolean setType(String newType)
	{
        ...
        String usrIn = newType.toLowerCase();
        if (usrIn.equals("grocery")){
            storeType = usrIn;
            validChecker = true;
        }
        else if (usrIn.equals("pet")){
            storeType = usrIn;
            validChecker = true;
        }
        else {
	...
	{
        boolean validOpen,validClose,openCloseValid;
        if (newOpen < 24 && newOpen > -1){
            validOpen = true;
        }
        else {
            validOpen = false;
        }
        if (newClose < 24 && newClose > -1){
            validClose = true;
        }
        else {
            validClose = false;
        }
        if (validOpen == true && validClose == true){
            openCloseValid = true;
            storeOpen = newOpen;
            storeClose = newClose;
        }
        else {
            openCloseValid = false;
        }
        return openCloseValid;
	}
	...
	public static String convertTime(int timeToConvert)
	{
        ...
        String convertedInt = "";
        if (timeToConvert > 12){
            convertedTime = timeToConvert - 12;
            convertedInt = Integer.toString(convertedTime);
            fullTime = convertedInt + "pm";
        }
        else if (timeToConvert == 12){
            convertedTime = timeToConvert;
            convertedInt = Integer.toString(convertedTime);
            fullTime = convertedInt + "pm";
        }
            ...
            convertedTime = 12;
            convertedInt = Integer.toString(convertedTime);
            fullTime = convertedInt + "am";
        }
        else if (timeToConvert < 12 && timeToConvert > 0){
            convertedTime = timeToConvert;
            convertedInt = Integer.toString(convertedTime);
            fullTime = convertedInt + "am";
        }
	...
	{
        String updatedOpen = convertTime(storeOpen);
        String updatedClose = convertTime(storeClose);
        String returnValue = storeType + " store is open from " + updatedOpen + " to " + updatedClose;
        return returnValue;
	}
